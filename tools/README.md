...
- `persistent-targets.tsv` declares the protocol, consumer, memory-report, namespace, lane, and input-model support of the four z-xml resident or streamed adapters. It has no output of its own.
...
- `zig/persistent.zig` owns Reader work over resident or streamed input. The `persistent-adapters` step builds raw-name, namespace-aware, default-Reader, and default-Reader namespace-summary executables. `check-generated-persistent.py`, `check-reader-scale`, and measurement commands pass the input model, consumer, iteration count, chunk size, and XML path; the executable writes one JSON result.
...
- `python/run-zebrac-matrix.py` owns correctness-qualified matched measurement. Repeated `--targets` and `--bin-dir` pairs place z-xml and peer commands in the same workload and lane group. It requires fresh common-summary or exact persistent qualification, bounds Zebrac time and output, and writes schema `z-xml-zebrac-matrix-v4` only after every selected run passes.
