...
- `python/check-generated-corpus.py` owns event and owned-Document qualification. Event mode checks declared event targets against the generated common summary. Document mode requires named DOM targets, selected valid shapes, and the z-xml Document oracle. It checks complete traversal summaries, construction and traversal mode agreement, retained-memory reports, and allocator closure. Both modes check every manifest file before applying the selection, bound each adapter process, and publish a result TSV only when every executed row passes.
...
The first two commands check the declared z-xml event and persistent modes. The remaining commands rebuild and check peer event, persistent, and owned-Document modes. Each command publishes results only after every executed row passes its exact oracle. Event and persistent modes record unsupported features as exclusions. Document mode treats a missing required feature as a failed match.
