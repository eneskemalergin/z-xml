...
- `persistent-targets.tsv` is the `z-xml-persistent-targets-v1` declaration entry for the five resident or streamed Reader adapters. `check-generated-persistent.py`, `run-zebrac-aa.py`, and `run-zebrac-matrix.py` read it; it writes nothing. `make -C ref check-generated-persistent` is its focused check.
...
- `zig/name_access.zig` enters through `z-xml-name-access`, built by `name-access`. It generates 64 records with 1, 16, or 64 attributes and repeated or unique names, parses them through the public Reader, and copies attributes before timing. It separately measures iteration, raw lookup, expanded lookup, and caller vocabulary classification over resident copies. It prints a human-readable Markdown sample table, not a component result protocol. `--verify` checks all 24 lanes without measurement samples and runs in the development test step. The probe is not included in the adapter union or package tests. Build with `zig build --build-file tools/build.zig name-access -Dtarget=x86_64-linux -Doptimize=ReleaseFast`, then run `tools/zig-out/bin/z-xml-name-access --verify` before `tools/zig-out/bin/z-xml-name-access`. One warmup and five samples use 256 traversals each, with rotating operation order; setup and output are not timed. This isolates access costs, not parsing speed or complete consumer throughput.
