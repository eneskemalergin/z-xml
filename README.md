...
The repository now contains the compiled reader type factory, lifecycle, specialized event and diagnostic shapes, whole-slice and buffered-I/O adapters, push drain, layout probe, and general ASCII-named element structure. The current reader accepts nested ordinary and empty elements across arbitrary chunks, matches closing names, preserves event lifetimes, and bounds active depth and open-name storage. This is implementation evidence, not a general XML parser claim. Attributes, text, Unicode, declarations, namespaces, encodings, DTD behavior, validation, and the tree follow the gates in [`plan/ROADMAP.md`](plan/ROADMAP.md).
