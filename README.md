...
The repository now contains the compiled reader type factory, lifecycle, specialized event and diagnostic shapes, whole-slice and buffered-I/O adapters, push drain, layout probe, XML 1.0 Fifth Edition names, streaming semantic UTF-8 text, numeric and predefined references, line-ending normalization, and atomic normalized source attributes. The current XML 1.0 UTF-8 no-DTD reader validates malformed UTF-8 and forbidden XML characters across arbitrary caller chunks while retaining only active element, start-tag, decoder-suffix, reference, and fixed fragment state. This is implementation evidence, not a complete XML parser claim. Declarations, comments, CDATA sections, processing instructions, namespaces, UTF-16, DTD behavior, validation, and the tree follow the gates in [`plan/ROADMAP.md`](plan/ROADMAP.md).
