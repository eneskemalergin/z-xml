//! Streaming UTF-8 XML output over a caller-owned `std.Io.Writer`.
//!
//! A writer builds exactly one document. Its state moves from the prolog, through
//! one root element, to the epilog and completion. Calls outside the current state
//! fail. The first mutation failure is retained, and later mutation calls return
//! that same error.
//!
//! Start tags stay in writer-owned memory until content starts or the element
//! closes. This lets namespace mode validate in-scope prefixes and duplicate
//! expanded attribute names before publishing a tag. Raw-name mode still checks
//! XML Name syntax but does not resolve namespaces. Bytes retained from names,
//! attributes, and namespace declarations are copied, so callers may reuse their
//! argument buffers after each call returns.
//!
//! Limits bound open structure, pending tags, namespace state, and retained
//! allocation capacity. They do not limit total output or sink storage. Capacity
//! is retained for reuse until deinitialization and is reported separately from
//! active counts. Every configured limit must be nonzero. A standalone declaration
//! value is valid only when declaration emission is enabled.
//!
//! The allocator and sink remain borrowed for the writer's lifetime. Deinitializing
//! the writer releases only writer-owned allocations and does not flush or
//! deinitialize the sink. An initialized writer must not be copied, used
//! concurrently, or entered recursively.
//!
//! The logical byte offset counts accepted UTF-8 output. It remains available after
//! validation, state, limit, or allocation failures, but becomes unknown after a
//! sink write fails. Output follows the selected XML 1.0 or XML 1.1 character and
//! namespace rules and is not pretty-printed.
